...
var _eventemitter3_4_0_4_eventemitter3 = createCommonjsModule(function (module) {
...
const Servers = new Proxy({}, {
    get: function (obj, prop) {
        if (!obj[prop]) {
            obj[prop] = {
                ServerE: new _eventemitter3_4_0_4_eventemitter3(),
                ClientE: new _eventemitter3_4_0_4_eventemitter3(),
            };
        }
        return obj[prop];
    },
...
class Client {
    ...
    async get({ server, path, data, }) {
        return await this.run(path, data, 'get', server);
    }
    /**
     * @TODO
    ...
    async post({ server, path, body, }) {
        return await this.run(path, body, 'post', server);
    }
    async run(path, data, type, server) {
        ...
        return await new Promise((res) => {
            ...
            let hasServer = ServerE.eventNames().findIndex((v) => v === serverEvName) > -1;
            if (!hasServer)
                return res(void 0);
            // 并发get
            let thisChannel = channel++;
            ClientE.once(`${type}+client+${server}+${path}:${thisChannel}`, (resData) => {
                res(resData);
            });
...
class Server {
    ...
    run(type, path, callback) {
        ...
        let noServer = ServerE.eventNames().findIndex((v) => v === serverName) === -1;
        if (!noServer && type === 'get') {
            throw new Error(`[Jinter Error]: Get Serer(${path}) only can be listening once, you should remove one`);
        }
        this.events.push({
            path,
            eid,
        ...
        ServerE &&
            ...
            ServerE.on(serverName, async ({ data, channel }) => {
                let events = this.events.filter((e) => e.serverName === serverName && e.off === false);
                if (events.length > 0) {
    ...
    onGet({ path }, callback) {
        return this.run('get', path, callback);
    }
    /**
     * @TODO
     * return a success or error flag
    ...
    onPost({ path }, callback) {
        return this.run('post', path, callback);
    }
    remove(type, path) {
        let name = this.prefixing(type, path);
        this.events = this.events.filter((v) => !(v.path === path && v.type === type));
